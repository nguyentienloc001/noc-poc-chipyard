...
last_session: "P1 bước 1-7 XONG TRONG 1 NGÀY. Bước 6-7: NoCResearchConfigs.scala fix API 1.13 (rocket pkg, SimpleTLNoCParams, TerminalRouter idiom — học từ MultiNoCConfig trong cây), cả 3 baseline build PASS (B2: 1m39s; B4: 1m35s; B8: pass, core rv64imac → E8 đúng, cần libgloss multilib trước khi đo 8-core); smoke B1-B5 5/5 PASS trên Baseline2CoreConfig Verilator (multi-hart RTL verified — E7 đóng; B4 +17.6% loaded latency = đo được contention; B5 106 c/RT; instret khớp spike). results.csv 6 rows đều flag smoke. Commits: f84dc3d, 1c1b3f0, 38df866, 2d1fe53 + sau đó."
next_action: "P1 bước 8 chuẩn bị + đo chính thức: (1) build libgloss multilib rv64imac/lp64 (trong container: cd /work/chipyard/toolchains/libgloss, rm -rf build, configure với --enable-multilib=\"rv64imac/lp64\" + CFLAGS specs picolibc như p0-step3-manual.sh do_libgloss, make install || true, verify libgloss_htif.a trong lib/rv64imac/lp64/); (2) smoke B3 NCORES=4 trên Baseline4CoreConfig (1 run, params giảm — xác nhận barrier 4 hart); (3) HỎI LOC chọn nơi chạy ma trận chính thức (M1: B1 full ~3-4h/run × 3 runs × nhiều config; CI x86 nhanh hơn nhưng private repo 2000 phút/tháng — cân nhắc public repo hoặc chia nhỏ); (4) chạy ma trận E1-E3 × B1-B5 × N_RUNS>=3 params full + IMAGE_DIGEST set, parse, tick docs/04; (5) report.md. Lưu ý benchmark 8-core: make MARCH=rv64imac MABI=lp64."
blockers: []
...
|---|---|---|---|
| 2026-06-11 (4) | Loc + Claude Code | P1 bước 1-7: audit Codex findings, infra harden, B1-B5 hoàn thiện + SECONDARY_ENTRY (E7 đóng: spike -p2 + Verilator RTL), bug gcc elide iterations bắt được, 3 baseline configs build pass, smoke 5/5 trên Baseline2 | Còn bước 8 (đo chính thức — cần Loc chốt M1 vs CI) + report.md |

...
last_session: "P1 bước 1-5 XONG (1 phiên): bước 1-2 (docs+Makefile standalone, run_sim/parser harden); bước 3-4 (B1 idx static; B2/B4/B5 viết mới; B3 SECONDARY_ENTRY) + bước 5 cơ chế multi-hart GIẢI QUYẾT: libgloss đưa hart phụ vào __main (weak wfi) → macro SECONDARY_ENTRY trong sync.h, verified spike -p2 (B3/B4/B5 cả 2 hart chạy, không deadlock). Bonus: bắt bug gcc -O2 elide iteration (copy 1/2 ITERS) → compiler barrier, ghi docs/03. Log: p1-step4-spike-smoke.log."
next_action: "P1 bước 6: copy src/chipyard-configs/NoCResearchConfigs.scala vào /work/chipyard/generators/chipyard/src/main/scala/config/ (KHÔNG sửa file chipyard khác), rồi build verilator 3 config: run_sim.sh Baseline2CoreConfig --build-only (tương tự 4/8). LƯU Ý: (a) file scala có import constellation.* — Baseline configs không dùng nhưng compile cả file cần constellation submodule (đã init ở P0 step3 manual: generators/constellation); (b) Baseline8CoreConfig dùng WithNSmallCores → benchmark cần MARCH=rv64imac MABI=lp64 + libgloss multilib rv64imac/lp64 (build thêm theo P0 report bài học #5) — kiểm tra march thật của small core trước (đọc đầu ra elaborate/dts); (c) build log tee vào results/raw/. Sau đó bước 7: smoke B1-B5 trên Baseline2CoreConfig (params giảm như spike smoke, N_RUNS=1, LOADMEM=1) — đây là verify Verilator multi-hart cuối cùng cho E7."
blockers: []
